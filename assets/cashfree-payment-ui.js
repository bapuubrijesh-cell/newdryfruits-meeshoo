(() => {
  const methods = [
    { id: "gpay", label: "G Pay", paymentMethod: "gpay", logo: "/assets/payment-logos/google-pay.svg" },
    { id: "phonepe", label: "PhonePe", paymentMethod: "phonepe", logo: "/assets/payment-logos/phonepe.png" },
    { id: "paytm", label: "Paytm", paymentMethod: "paytm", logo: "/assets/payment-logos/paytm.png" },
    { id: "bhim", label: "BHIM UPI", paymentMethod: "bhim", logo: "/assets/payment-logos/bhim.svg" },
    { id: "whatsapp", label: "WhatsApp Pay", paymentMethod: "upi", logo: "/assets/payment-logos/whatsapp.svg" },
  ];
  ...
  function setLoading(ui, loading) {
    ...
    ui.qrButton.disabled = loading;
    ui.payButton.classList.toggle("is-loading", loading);
    ui.payButton.querySelector(".direct-upi-pay-copy").textContent = loading ? "Opening..." : "PayNow";
  }
  ...
  async function startPayment(ui) {
    let checkout;
    try {
    ...
    if (typeof window.startCashfreeCheckout !== "function") {
      showStatus(ui, "error", "Payment unavailable", "Please refresh the page and try again.");
      return;
    }

    hideStatus(ui);
    await window.startCashfreeCheckout(ui.payButton);
  }

  function showCashfreePaymentUi() {
    if (location.pathname !== "/payment") return;
    const page = document.querySelector(".pm-main");
    if (!page || page.dataset.cashfreePaymentUiReady) return;
    page.dataset.cashfreePaymentUiReady = "true";
    ...
    section.innerHTML = `
      <div class="direct-upi-page">
        <header class="direct-upi-topbar">
          ...
          <span class="direct-upi-wordmark">meesho</span>
        ...
        <div class="direct-upi-upi-title"><span class="direct-upi-upi-chip">UPI</span><span>UPI (GPay/PhonePe/Paytm)</span></div>
        <div class="direct-upi-methods"></div>
        <button class="direct-upi-qr" type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3h7v7H3V3Zm2 2v3h3V5H5Zm9-2h7v7h-7V3Zm2 2v3h3V5h-3ZM3 14h7v7H3v-7Zm2 2v3h3v-3H5Zm9-2h3v3h-3v-3Zm4 0h3v7h-3v-7Zm-4 4h3v3h-3v-3Z"/></svg><span>Scan QR Code to Pay</span><span>›</span></button>
    ...
    ui.qrButton.addEventListener("click", () => {
      selectedMethod = { id: "upi", label: "UPI app", paymentMethod: "upi", logo: "" };
      ui.qrButton.classList.add("is-selected");
      hideStatus(ui);
      renderMethods();
    });
    ui.payButton.addEventListener("click", () => startPayment(ui, selectedMethod));
    ui.status.querySelector(".direct-upi-retry").addEventListener("click", () => startPayment(ui, selectedMethod));

  }

  new MutationObserver(showCashfreePaymentUi).observe(document.documentElement, { childList: true, subtree: true });
